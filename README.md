...
Minimal CLI-based job queue (internship assignment solution scaffold).

This project implements a job queue with:
...
- CLI (commands implemented via `src/cli.ts`).

Notes
-----
This repository contains the core building blocks. To finish the assignment run:
...
```cmd
node dist/cli.js enqueue "{\"id\":\"job1\",\"command\":\"echo hi\"}"
```

4. Start a worker:

```cmd
node dist/cli.js worker:start --count 1
```
...
6. Run the built-in verify script (simple smoke test):
...
What I changed
--------------
- Added `src/cli.ts` - CLI commands using `commander`.
- Added `src/utils.ts` - PID file helpers.
- Improved `src/db.ts` transaction safety and DLQ typing.
